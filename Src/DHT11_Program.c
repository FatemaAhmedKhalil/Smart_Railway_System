...
static u32 u32TimeInstance1 = 0, u32TimeInstance2 = 0, u32PulseDuration = 0;
...
u8 DHT11_voidRawDataUpdate()
{
...

    {
        ...
        u32TimeInstance1 = MTIM_u32ReadCounterValue(DHT11_TIM_NUM);
        do
        {
            u8TempReading = DHT11_DATA_FastRead;
        }
        while( u8TempReading == GPIO_HIGH && MTIM_u8ReadUpdateInterrupt(DHT11_TIM_NUM) == 0 );
        u32TimeInstance2 = MTIM_u32ReadCounterValue(DHT11_TIM_NUM);
        
        u32PulseDuration = u32TimeInstance2 - u32TimeInstance1;
        if (u32PulseDuration > 42  || MTIM_u8ReadUpdateInterrupt(DHT11_TIM_NUM) == 1) // add an error range of 2usec
        {
        ...
        u32TimeInstance2 = MTIM_u32ReadCounterValue(DHT11_TIM_NUM);
        u32PulseDuration = u32TimeInstance2 - u32TimeInstance1;
        if (u32PulseDuration > 82  || MTIM_u8ReadUpdateInterrupt(DHT11_TIM_NUM) == 1) // add an error range of 2usec
        {
        	// Time Out, Means Not working
            return 1; // Exit with error
        }
        ...
        u32PulseDuration = u32TimeInstance2 - u32TimeInstance1;
        if (u32PulseDuration > 90 || MTIM_u8ReadUpdateInterrupt(DHT11_TIM_NUM) == 1 ) // add an error range of 2usec
        {
        	// Time Out, Means Not working
            return 1; // Exit with error
        }
...
u8 DHT11_u8Decoder(void)
{
    ...
    while (u8TempReading == GPIO_LOW && MTIM_u8ReadUpdateInterrupt(DHT11_TIM_NUM) == 0);
    u32TimeInstance1 = MTIM_u32ReadCounterValue(DHT11_TIM_NUM); //The first instance where the input was HIGH
    ...
    while (u8TempReading == GPIO_HIGH && MTIM_u8ReadUpdateInterrupt(DHT11_TIM_NUM) == 0);
    u32TimeInstance2 = MTIM_u32ReadCounterValue(DHT11_TIM_NUM); // The last  instance where the input was HIGH
    u32PulseDuration = u32TimeInstance2 - u32TimeInstance1;

    // The average between 30 and 70 is 50, So this will be the optimal value to compare at
    if (u32PulseDuration > 35  || MTIM_u8ReadUpdateInterrupt(DHT11_TIM_NUM) == 1)
        return 1;
    else
        return 0;
}
