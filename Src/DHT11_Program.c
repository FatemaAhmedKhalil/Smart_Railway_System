 ...
 * 
 */
//Shared Local Variables for this program
...
void DHT11_voidInitialize(void)
{
    ...
    //2==Turn On the RCC for desired Timer
    ...
    //4==Setting Timer Up but not start it
    MTIM_voidSetPrescaler(TIM2,DHT11_TIM_PSC_1MHz);	    //Set the prescaler
...
u8 DHT11_voidRawDataUpdate()
{
    ...
    //Converting the GPIO Pin to Input with Pullup resistor mode
...

    {
        ...
        //This will take up to 4960 micro-seconds to fully peform this section.
        ...
        //If the sensor was working, It will now start receinving data.
        for (u8 i = 0; i<40; i++)
        {
            DHT11_DATA[i/8]<<=1;
...
u8   DHT11_u8Decoder(void)
{
    ...
    //Me 3 months later: Nothing is more Permenant than a temporary solution lol,
...
//
//    //Preload New Prescale and autoreload values
